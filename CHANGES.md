...
## 2026-09-24 — squirrel-acorn-bowling, whale-rainbow-fountain (총 276개)
- **[🐿️ 아기 다람쥐의 도토리 굴리기 대작전 (squirrel-acorn-bowling)]** — 귀여운 아기 다람쥐와 함께 황금 도토리를 굴려 솔방울 핀들을 퐁퐁 쓰러뜨리는 숲속 볼링 터치 아케이드 (외부 의존성 0).
- **[🐳 아기 고래의 오색 무지개 물뿜기 (whale-rainbow-fountain)]** — 푸른 바다의 귀여운 아기 고래가 오색 무지개 물줄기를 뿜어 하늘의 별구름과 음표 방울을 터뜨리는 힐링 타이밍 아케이드 (외부 의존성 0).

## 2026-09-23 — otter-river-clam-catch, dragon-baby-fire-fruit (총 274개)
- **[🦦 아기 수달의 퐁당퐁당 조개 잡기 (otter-river-clam-catch)]** — 맑은 시냇물에서 조개와 진주조개를 쏙쏙 잡고 통나무와 바위를 피하는 힐링 터치 아케이드 (외부 의존성 0).
- **[🐲 아기 용의 말랑말랑 불꽃 과일 굽기 (dragon-baby-fire-fruit)]** — 아기 용의 따뜻한 불꽃으로 마시멜로와 과일을 노릇노릇 알맞게 굽는 힐링 타이밍 아케이드 (외부 의존성 0).
...
## 2026-09-02 — ocean-pearl-diver, monster-cookie-feed
- **[🌊 해저 진주 조개 찾기 (ocean-pearl-diver)]** — 조개 터치 진주 수집 액션 (외부 의존성 0)
- **[🍪 아기 몬스터 쿠키 먹이기 (monster-cookie-feed)]** — 쿠키 맞춤 수집 먹이기 (외부 의존성 0)

### 2026-08-31
- 🚀 **우주 운석 대시 (Cosmic Meteor Dash)**: 쏟아지는 불타는 운석을 피하며 별 보석과 보호막을 수집하는 스릴 만점 우주선 비행 회피 게임 추가.
- 🔔 **멜로디 크리스탈 차임 (Melody Crystal Chime)**: 5가지 맑고 영롱한 펜타토닉 크리스탈 차임벨을 터치해 멜로디를 연주하는 리듬 액션 게임 추가.
...
## 2026-08-28 — turtle-bubble-rescue, cosmic-star-baker (총 232개)
...
- 가장자리까지 열려 있는 넓은 빈 공간은 내 땅으로 단정하지 않도록 처리해, 시작 직후 한 돌만 둬도 큰 점수로 잘못 계산되는 오류를 함께 차단.
- 게임 중 숫자는 `잡은 돌`로 명확히 표시하고, 종료 화면에는 돌·내 땅의 계산 내역을 함께 표시.
- `📖 바둑말` 가이드 추가: 돌, 숨 쉴 자리, 잡기, 내 땅, 쉬기, 프로 화면의 AI 형세값을 어린이 말로 설명.
- **끝내기 흐름 보완**: `🏁 점수 세기`로 빈칸이 남아도 종료할 수 있고, AI 대국에서 사용자가 `쉬기`를 누르면 잡기·살리기 같은 급한 일이 없을 때 AI도 쉬어 연속 패스로 끝난다. 빈칸을 끝까지 메우지 않는다.
