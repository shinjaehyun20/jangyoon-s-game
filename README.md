...
- **🍄 슈퍼 점프 모험 추가 (2026-06-14)** — 달리고 점프해서 코인을 모으고 적(굼바)을 위에서 밟아 처치, 깃발에 도착하면 클리어하는 캔버스 기반 옆스크롤 플랫포머. 타일맵 레벨(땅·벽돌·물음표 블록·파이프·구덩이), 중력/관성 물리, 카메라 추적, 패럴럭스 배경(구름·언덕), 키보드(←→·스페이스)와 모바일 터치 버튼 동시 지원. dino-run(탭 점프 회피)·cloud-jump(수직 상승)과 달리 좌우 자유 이동 + 블록 상호작용 + 적 밟기 + 깃발 골인의 정통 플랫포머 메커니즘 특화. 시간 제한 없는 완주형. localStorage 키: `super-mario_best`.
- **🌀 미로 탈출 시간 제한 제거 (2026-06-14)** — 60초 카운트다운으로 게임이 강제 종료되던 방식을 없애고, 시계에 쫓기지 않고 자기 페이스로 모든 난이도(7×7→13×11) 미로를 완주하는 방식으로 변경. 타이머는 "걸린 시간" 카운트업 기록으로 바뀌고, 최고 기록은 가장 빠른 완주 시간으로 저장. 순수 타임어택/생존형 게임들의 30초·60초 제한은 의도된 핵심 룰이라 그대로 유지. localStorage 키: `maze-escape_besttime`.
