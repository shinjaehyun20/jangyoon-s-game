...
| 게임 | 설명 | 카테고리 |
|---|---|---|
| [🍄 슈퍼 점프 모험 (super-mario)](./super-mario) | 3스테이지+👑보스의 옆스크롤 플랫포머! 버섯·🌸불꽃·⭐별 파워업, 숨은 블록·파이프 비밀길, 방향패드 조작 | 액션/플랫포머 |
...
원작 슈퍼마리오를 벤치마킹한 캔버스 옆스크롤 플랫포머. **월드 1 = 스테이지 1-1·1-2·1-3 + 👑 보스전**으로 구성.
- **방향패드(◀▶▲▼)** + 점프 + 상황별 🔥슈팅 버튼. ▼로 파이프 진입, ▲로 복귀 (키보드도 지원)
- **파워 티어**: 작은 마리오 → 🍄버섯(큰 마리오) → 🌸꽃(파이어). 적에게 맞으면 한 단계만 하락(원작식)
- **🔥 파이어볼**: 꽃을 먹으면 바닥을 튕기는 불꽃을 발사해 적을 처치
- **⭐ 무적 별("superduper")**: 일정 시간 무지개로 반짝이며 적을 접촉만으로 처치 + 속도 업
- **숨은 블록**: 빈 곳을 머리로 치면 나타나는 히든 코인/파워업
- **파이프 비밀길**: 워프 파이프(▼)로 들어가는 코인 보너스 룸
- **👑 보스전**: HP 바를 가진 보스를 파이어볼/밟기로 무찌르면 월드 클리어
- 가변 점프, 스테이지 테마(들판·동굴·성), WebAudio 효과음, 구덩이 추락 시 안전 부활, 목숨 3개
- 최고 점수 `localStorage` 저장(`super-mario_best`). 헤드리스 하니스로 진행·전투·보스·워프·히든블록 검증
