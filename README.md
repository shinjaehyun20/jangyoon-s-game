...
- **🌦️ 날씨 옷장 탐험 · 🪞 마음 거울 추가 (2026-07-23)** — 날씨 옷장 탐험은 햇빛·비·눈·바람·더위·추위 장면을 보고 알맞은 옷을 고르는 45초 생활 판단 게임이며, 마음 거울은 생활 장면을 읽고 가장 가까운 감정 표정을 고르는 8라운드 사회정서 게임입니다. 두 게임 모두 오답 감점 없이 힌트를 주고 다시 선택하게 하며, 단일 HTML·SVG 썸네일·100dvh/overflow 보호·localStorage 최고기록·외부 의존성 0건을 포함합니다.
...
- **🪨 공기놀이 · 👣 사방치기 콩콩 추가 (2026-07-13)** — 공기놀이: 돌 하나가 포물선(4×APEX×t×(1-t))을 그리며 솟았다 손으로 떨어지는 동안, 바닥에 흩어진 공기알을 전부 톡톡 모으고 돌이 손에 닿는 마지막 캐치 윈도우(700ms→350ms로 점점 짧아짐)에서 [잡기]를 탭해야 하는 전통놀이, 라운드마다 모아야 할 알이 1개→5개로 늘고 비행 시간은 3.2초→2.0초로 짧아짐, 알을 다 못 모으고 잡으면 "돌은 잡았지만" 실패 처리, 5판 도전, localStorage 최고 성공 수(`gonggi-nori_best`). 사방치기 콩콩: 좌우로 튕기는 마커가 목표 칸 구간에 있을 때 [던지기]를 탭해 돌을 놓는 조준 단계(오조준 시 3회까지 재시도) 후, 목표 칸만 피해 나머지 칸을 순서대로 콩콩 밟는 홉 단계(제한시간 1.8초→1.0초로 단축, 목표 칸을 밟으면 즉시 실패)로 이어지는 2단계 전통놀이, 5판 도전, localStorage 최고 성공 수(`sabangchigi-hop_best`). 두 게임 모두 기존 jegi-kick(드래그 재포지셔닝+타이밍 캐치 단일단계)·ddakji-flip(스와이프 속도 판정)과 달리 각각 "동시다발 수집 후 타이밍 캐치"와 "조준+순서 탭 2단계 복합" 메커니즘으로 차별화, 단일 HTML + CSS 애니메이션 + SVG 썸네일, 외부 의존성 0건. 검증은 PowerPoint COM 대신 Node vm DOM mock 하네스로 happy-path(5판 전승) + 실패 경로(오조준 소진·목표칸 밟음·제한시간 초과·오탭) 전체 실행, 런타임 예외 0건.
- **🪢 줄넘기 놀이 · 🪨 비석치기 걷기 추가 (2026-07-12)** — 줄넘기 놀이: 원형으로 돌아가는 줄이 발밑 노란 구역(feet-zone)에 닿는 순간 화면을 탭해서 폴짝 뛰어넘는 리듬 타이밍 게임, 5회 성공마다 줄 회전 속도가 빨라짐, 너무 빠르거나 늦게 탭하면 줄에 걸려 목숨 감소(3개), localStorage 최고 콤보(`jump-rope-rhythm_best`). 비석치기 걷기: 전통놀이 비석치기를 모티브로, 머리 위 돌이 좌우로 무작위 기울어지는 것을 반대쪽 "톡" 버튼으로 눌러 균형을 맞추며 목표 지점까지 걸어가는 게임, 라운드가 오를수록 기울기 변화가 빨라짐, 균형이 무너지면 돌이 떨어지지만 감점 없이 다음 판 진행(좌절 방지), 5판 도전, localStorage 최고 성공 수(`stone-balance-walk_best`). 두 게임 모두 기존 ddakji-flip(스와이프 속도 판정)·jegi-kick(드래그 재포지셔닝)·top-spin-duel(연타 차징)과 달리 각각 "회전 주기 타이밍 탭"과 "좌우 교차 보정 균형 유지" 메커니즘으로 차별화, 단일 HTML + CSS 애니메이션 + SVG 썸네일, 외부 의존성 0건.
- **🎴 딱지치기 · ⚖️ 저울 균형 맞추기 추가 (2026-07-10)** — 딱지치기: 손가락으로 내 딱지를 힘차게 아래로 스와이프하면 이동 속도(px/ms 순간 최고 속도)를 게이지로 환산해 판정하는 전통놀이 게임, 라운드별 요구 속도가 22%→68%로 점점 높아짐, 성공 시 상대 딱지가 3D `rotateY` 플립 + 바람(💨) 파티클 연출, 실패해도 감점 없이 다음 판 진행(좌절 방지), 5판 도전, localStorage 최고 성공 수(`ddakji-flip_best`). 저울 균형 맞추기: 숫자 블록을 드래그해 저울 오른쪽 판에 담으면 왼쪽 목표 숫자와의 차이만큼 빔이 기울고(CSS `rotate`), 합이 정확히 같아지면 균형을 이루는 덧셈 퍼즐, 잘못 담긴 블록은 탭 한 번으로 다시 빼낼 수 있어 시행착오 허용, 시간 제한 없이 5레벨(목표값 5→15) 도전, localStorage 레벨 완주 기록(`scale-balance_best`). 두 게임 모두 기존 jegi-kick(드래그 재포지셔닝+타이밍 캐치)·top-spin-duel(반복 탭 차징+자원관리)·star-adder(탭으로 그룹 선택)와 달리 각각 "스와이프 속도 판정"과 "드래그 합산+물리 피드백(빔 기울기)" 메커니즘으로 차별화, 단일 HTML + DOM/CSS 애니메이션 + SVG 썸네일, 외부 의존성 0건.
- **🪁 제기차기 · 🪀 팽이 돌리기 대결 추가 (2026-07-09)** — 제기차기: 손가락 드래그로 아이를 좌우로 움직여 좌우로 흔들리며 떨어지는 제기(전통 놀이기구, 오방색 술 장식)를 발밑에서 차올리는 게임. 킥에 성공할 때마다 다음 낙하 위치가 랜덤하게 바뀌어 계속 재포지셔닝이 필요하고, 연속 성공할수록 낙하 속도·좌우 흔들림 폭이 점점 커짐, 놓치면 즉시 종료, localStorage 최고 연속 기록(`jegi-kick_best`). 팽이 돌리기 대결: 화면을 빠르게 탭해 3초 동안 내 팽이의 회전 파워를 채우고, 대결이 시작되면 서서히 줄어드는 파워를 상대보다 오래 유지해야 승리, 팽이를 탭하면 부스트(라운드당 3회 한정, 쿨다운 있음)로 파워를 보충 가능, 5판 승부로 진행, localStorage 최고 승수(`top-spin-duel_best`). 두 게임 모두 한국 전통 놀이(제기차기·팽이치기)를 소재로 삼아 기존 bounce-ball(고정 위치 탭 드리블)·rocket-launch(좌우 게이지 타이밍)·snail-race(단순 연타 경주)와 달리 각각 "드래그 재포지셔닝+타이밍 캐치"와 "사전 차징+제한적 실시간 부스트 자원관리" 메커니즘으로 차별화, 단일 HTML + Canvas 2D + SVG 썸네일, 외부 의존성 0건.
- **🏹 화살 풍선 맞추기 · 🫧 비눗방울 잡기 추가 (2026-07-08)** — 화살 풍선 맞추기: 화면 아래쪽을 눌러 위로 드래그하면 활이 당겨지고 손을 떼면 화살이 포물선(중력 적용)으로 발사되는 조준 게임, 떠오르는 풍선을 맞히면 점수·폭탄 풍선(💣)은 콤보 초기화·황금 풍선(★)은 25점 고정 보너스, 화살 8발 안에 최고점 도전, localStorage 최고점(`archery-balloon_best`). 비눗방울 잡기: 손가락으로 채집망을 드래그해 아래에서 위로 흔들리며(wobble) 떠오르는 비눗방울을 잡는 게임, 방울 크기가 작을수록 고득점(반비례 점수), 무지개 그라데이션 테두리 방울은 60점 왕대박, 40초 도전, localStorage 최고점(`soap-bubble-catch_best`). 두 게임 모두 기존 ring-toss(고정 탭 발사)·ocean-cleanup(드래그 수집)과 달리 각각 "드래그-당김-포물선 발사"와 "실시간 드래그 추적 채집"으로 조작 메커니즘 차별화, 단일 HTML + SVG 썸네일, 외부 의존성 0건.
- **🦆 오리 목욕 놀이 · 🐶 강아지 뼈다귀 찾기 추가 (2026-07-07)** — 오리 목욕 놀이: 욕조에 둥둥 뜬 오리(🦆)를 손가락으로 콕 눌러 점수를 모으는 탭 반사 게임, 반짝이는 황금 오리는 5점 보너스, 잡을 때 물방울 튀김(splash) 애니메이션 피드백, 30초 타임어택, localStorage 최고점(`rubber-duck-bath_best`). 강아지 뼈다귀 찾기: 4×4 흙더미 그리드 16칸 중 뼈다귀(🦴) 6개가 숨어있어 손가락으로 파헤쳐 모두 찾는 기억력·탐색 게임, 두더지(🦔) 칸을 파면 감점, 뼈다귀 전부 찾으면 즉시 클리어, 40초 도전, localStorage 최고 발견 수(`puppy-bone-hunt_best`). 두 게임 모두 기존 whack-a-mole(즉시 사라지는 탭)·memory-match(카드 뒤집기)와 달리 각각 "고정 위치 탭 잡기"와 "그리드 파헤치기" 메커니즘으로 차별화, 단일 HTML + SVG 썸네일, 외부 의존성 0건.
- **🧶 고양이 낚싯대 놀이 · ✈️ 종이비행기 날리기 추가 (2026-07-06)** — 고양이 낚싯대 놀이: 화면을 드래그해 깃털을 움직이면 고양이가 부드럽게 따라와(lerp 추적) 실뭉치🧶를 낚아채는 드래그 유도 게임, CSS `perspective`+`rotateX` 원근 방바닥, 실뭉치는 일정 시간 안에 못 잡으면 자연스럽게 사라졌다 재등장, 40초 도전, localStorage 최고점(`cat-teaser-play_best`). 종이비행기 날리기: 화면을 톡톡 터치할 때마다 위로 솟구치는 중력+플랩(flap) 물리 비행 게임, ⭐별은 점수 획득·🌩️먹구름은 목숨 감소(3개, 실패해도 계속 진행)로 좌절 없는 난이도, 비행기는 상승/하강 속도에 따라 기울어지는 회전 피드백, 40초 도전, localStorage 최고점(`paper-airplane-fly_best`). 두 게임 모두 기존 드래그 유도(바다 청소부)·터치 반사(hamster-wheel-run)류와 겹치지 않도록 각각 "따라오게 만들기"와 "탭 플랩 비행" 메커니즘으로 차별화, 단일 HTML + SVG 썸네일, 외부 의존성 0건.
- **🎯 몬스터 파크 추가 (2026-07-05)** — 공원에 나타나는 몬스터를 탭해서 포획하는 수집형 게임. 12종 몬스터(등장 확률 가중치 차등, 희귀할수록 낮음)가 랜덤 위치에 짧게 나타났다 사라지고, 탭하면 포획 애니메이션 + 도감(dex) 등록. 같은 몬스터 3마리를 모으면 레벨업, 12종 완전 수집이 목표. 45초 타임어택, localStorage에 도감 진행도(`monster-park_dex`)와 최고 포획 수(`monster-park_best`) 저장. 기존 whack-a-mole류와 달리 랜덤 등장 개체를 즉시 판별·수집하는 인지+반사 복합 메커니즘.
- **🌀 터널 달리기 · 🤿 다이빙대 점프 추가 (2026-07-04)** — CSS 3D(perspective+preserve-3d+translateZ) 원근 기법을 신규 적용한 2종. 터널 달리기: `perspective` 원근 카메라 + 3D 링이 translateZ로 화면을 향해 다가오는 터널 러너, 좌우 3레인 이동으로 장애물(🪨)은 피하고 보석(💎)은 획득, 30초 도전. 다이빙대 점프: 다이빙대에서 좌우로 스윕하는 물결 링에 타이밍 맞춰 점프해 착수 정확도로 점수를 얻는 게임, `rotateX` 원근 수면 + 낙하 시 translateZ 확대로 카메라에 가까워지는 착시, 5회 도전. 두 게임 모두 runner-game/space-catcher의 레이어 분리(그림자·측면·정면·글로우) 3D 캐릭터 카드 기법을 재사용.
- **🐹 햄스터 쳇바퀴 달리기 · 🐞 무당벌레 잎사귀 건너기 추가 (2026-07-03)** — 햄스터 쳇바퀴 달리기: 화면을 빠르게 탭할수록 쳇바퀴가 회전하고 거리가 쌓이는 탭 스피드 게임. 지나가는 도토리(🌰)를 탭하면 보너스 거리, 탭을 멈추면 감속. 무당벌레 잎사귀 건너기: 반짝이는 다음 잎사귀만 순서대로 탭해야 무당벌레가 폴짝 뛰어 건너가는 순서 판단 게임. 잘못된 잎사귀를 탭하면 풍덩 빠져 목숨 감소(목숨 3개), 기존 frog-jump(1버튼 타이밍)와 달리 다중 타겟 중 정답 선택이 핵심.
