...
| 게임 | 설명 | 카테고리 |
|---|---|---|
| [🍄 슈퍼 점프 모험 (super-mario)](./super-mario) | 달리고 점프해서 코인 모으고 적을 밟아요! 깃발 도착하면 클리어하는 옆스크롤 플랫포머 (키보드+터치) | 액션/플랫포머 |
...
달리고 점프해서 코인을 모으고 적을 밟아 처치, 깃발에 도착하면 클리어하는 캔버스 기반 옆스크롤 플랫포머.
- 키보드(← → 이동 · 스페이스/위 점프)와 모바일 터치 버튼(◀ ▶ · 점프) 동시 지원
- **가변 점프**: 점프 버튼을 오래 누르면 더 높이 뛰어요
- 타일맵 레벨(약 132칸): 땅·벽돌·물음표/파워업 블록·파이프·구덩이, 머리로 블록 치기 / 굼바(👾) 밟기
- **🍄 슈퍼버섯 파워업**: 파워업 블록(★)을 치면 버섯이 나오고, 먹으면 커져서 적에게 한 번 맞아도 작아지기만 해요(실드)
- **WebAudio 효과음**: 점프·코인·블록·적 처치·파워업·클리어 사운드 인라인 합성
- 구덩이에 빠지면 마지막 안전 위치로 부활, 목숨 3개
- 시간 제한 없는 완주형(깃발 골인=클리어), 최고 점수 `localStorage` 저장(`super-mario_best`)
